<template>
  <div v-if="visible" class="modal-overlay">
    <div class="modal-content rounded-xl flex flex-col relative bg-stone-50">
      ...
      <div :class="['image-container', { 'expanded': isImageExpanded }]" class="relative">
        <img :src="deal.image" alt="Deal Image" :class="['deal-image', { 'rounded-t-xl': !isImageExpanded }, { 'rounded-xl': isImageExpanded }]" />
        <button class="expand-button" @click="toggleImage">
          <v-icon :name="isImageExpanded ? 'gi-contract' : 'gi-expand'" 
      ...
      <div class="details p-4 flex flex-col justify-between">
        <div class="flex flex-col flex-grow">
          <div class="flex flex-row justify-between">
            ...
            <span class="inline-flex shrink-0 items-center rounded-full bg-green-50 px-2 py-0.5 text-lg font-medium text-green-700 ring-1 ring-green-600/20 ring-inset">{{ deal.distance }} km</span>
          </div>
      ...
      <div class="map bg-green-200 rounded-b-xl flex-none">
        <iframe class="w-full h-full rounded-b-xl" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://maps.google.com/maps?width=600&amp;height=400&amp;hl=en&amp;q=University of Oxford&amp;t=&amp;z=14&amp;ie=UTF8&amp;iwloc=B&amp;output=embed"></iframe>
      </div>
    </div>
  </div>  
</template>
